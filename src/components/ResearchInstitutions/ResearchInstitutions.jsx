...
export default function ResearchInstitutions() {
  ...
  const fetchInstitutions = async () => {
    try {
      ...
      const data = await fetchData('document', params, 100)
  ...
  return (
    <LayoutHistorianWorkshop pageTitle={t('menuItems.research_institutions')}>
      ...
      <div className='lg:overflow-scroll'>
        <div className='grid grid-cols-12 gap-[20px] pt-[40px] pb-[100px] lg:pb-[40px]'>
          {results?.map((item, index) => {
            return <CardLink key={index} data={item} />
          })}
        </div>
      </div>
      ...
      {/* MOBILE: BTN MENU */}
      <div
        className='lg:hidden fixed bottom-0 left-0 right-0 z-[100] h-[70px] w-full flex border-t border-black'
        style={{
      ...
      <div
        className={classNames(
          'lg:hidden h-[360px] fixed bottom-[70px] left-0 right-0 bg-paper border-black border-t transition-all duration-[750ms]',
          {
