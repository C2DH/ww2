...
const reduceUrlWithEllipsis = (url, maxLength = 30) => {
  // remove http:// or https://
  const _url = url.replace(/(^\w+:|^)\/\//, '')
...
export default function Source({ data, handleSourcePopup }) {
  ...
  const isPDF =
    data.attachment && data.attachment.split('.').pop().toLowerCase() === 'pdf'
  ...
  return (
    <div
      ...
      className={classNames('w-full h-full lg:relative lg:top-0 absolute', {
    ...
    >
      ...
      <div
        className={classNames(
          'hidden lg:block absolute top-[40px] left-[40px] text-[30px] text-white z-[99]',
          {
      ...
      <div className='h-[120px] lg:hidden flex justify-center items-center border-b border-white relative z-[100] bg-[rgba(0,0,0,0.9)]'>
        <span
          className='text-white uppercase text-[24px] cursor-pointer'
          onClick={handleSourcePopup}
        >
          {t('close')}
        </span>
      ...
      <div
        className={classNames(
          'container mx-auto relative px-[20px] overflow-scroll',
          {
            'h-[calc(100dvh-120px)] sm:h-[calc(100vh-120px)]':
              pathname !== '/spatiotemporal-map',
            'h-[calc(100dvh-120px)] sm:h-[calc(100vh-80px)]':
              pathname === '/spatiotemporal-map',
          }
        )}
      >
          ...
          <div className='col-span-12 lg:col-span-7 lg:col-start-2'>
            <div className='pb-[20px] pt-[20px] 2xl:pb-[40px] 2xl:pt-[80px] relative'>
              ...
              {(data.type === 'image' || data.type === 'photo') &&
                ...
                !isPDF && (
                  <ImageZoom
                    image={rootPath + data.data.resolutions.preview.url}
                    alt={data.title}
                  />
                )}
              ...
              {/** VIDEO */}
              {data.type === 'video' &&
                data.data?.videoResolutions?.hsl?.url && (
                  <Player
                    url={data.data.videoResolutions.hsl.url}
                    controls={true}
                    status={'video'}
                  />
                )}
              ...
              {isPDF && (
                <>
                  <ArrowLeftCircleIcon
                    ...
                    onClick={prevPage}
                    className={classNames(
                      'absolute -top-[10px] 2xl:top-[40px] left-[0px] cursor-pointer',
                      {
                  ...
                  <ArrowRightCircleIcon
                    ...
                    onClick={nextPage}
                    className={classNames(
                      'absolute -top-[10px] 2xl:top-[40px] left-[50px] cursor-pointer',
                      {
                  ...
                  <div className='zoom-controls'>
                    <MagnifyingGlassMinusIcon
                      ...
                      onClick={zoomOut}
                      className={classNames(
                        'hidden lg:block absolute -top-[10px] 2xl:top-[40px] left-[200px] cursor-pointer transition-all duration-500',
                        {
                    ...
                    <MagnifyingGlassPlusIcon
                      ...
                      onClick={zoomIn}
                      className={classNames(
                        'hidden lg:block absolute left-[250px] -top-[10px] 2xl:top-[40px] cursor-pointer transition-all duration-500',
                        {
          ...
          <div className='col-span-12 lg:col-span-3 lg:col-start-10 lg:border-l text-white overflow-scroll lg:pr-[30px]'>
            {data.covers?.map((item) => {
              if (item.type === 'glossary') {
                return (
                  <div key={item.id}>
                    <h1 className='lg:pl-[25px] text-[32px] font-semibold pt-[30px] pb-[30px]'>
                      {item.data.title[language]}
                    </h1>
                    <hr className='w-1/2' />
                    <p className='lg:pl-[25px] text-[28px] font-light pt-[30px] pb-[30px]'>
                      {item.data?.description[language]}
                    </p>
                  </div>
                )
              }
            ...
            {(data.type === 'book' ||
              ...
              <>
                <h1 className='lg:pl-[25px] text-[32px] font-semibold pt-[30px] pb-[30px]'>
                  {data.data.zotero.title}
                </h1>
                <hr className='w-1/2' />
                <span className='block lg:pl-[25px] text-[28px] font-light pt-[30px] pb-[30px]'>
                  Publisher :{' '}
                  {data.data.zotero.publisher
                    ? data.data.zotero.publisher
                    : 'NC'}
                </span>
                <hr className='w-1/2' />
                <Link
                  to={data.data.zotero.url}
                  target='_blank'
                  className='block lg:pl-[25px] text-[28px] font-light pt-[30px] pb-[30px]'
                >
                  Lien :{' '}
                  <span className='hover:text-blue transition-all duration-500'>
                    {data.data.zotero.url ? data.data.zotero.url : 'NC'}
                  </span>
                </Link>
              </>
            )}
            ...
            {typeof data.url === 'string' && data.url.length > 0 && (
              <div className='block lg:pl-[25px] text-[28px] font-light pt-[30px] pb-[30px] '>
                <ArrowRightIcon
                ...
                >
                  {reduceUrlWithEllipsis(data.url)}
                </a>
              </div>
            )}
          </div>
        ...
        <>
          <TransformComponent>
            <img
              src={image}
              alt={alt}
              className='w-full max-h-[350px] lg:max-h-[600px] 2xl:max-h-[650px] object-contain'
            />
          </TransformComponent>
      ...
      <div className='flex cursor-pointer'>
        ...
        >
          <MagnifyingGlassMinusIcon
            style={{ width: '50px' }}
            className={classNames(
              'text-white px-[15px] py-[6px] lg:py-[12px]',
              { 'pointer-events-none opacity-20': zoom === null || zoom === 1 }
            )}
          />
        </div>
        <div
          className='uppercase text-[15px] lg:text-[20px] text-white flex items-center border-t border-b border-white px-[12px]'
        ...
        >
          <MagnifyingGlassPlusIcon
            ...
            className={classNames(
              'text-white px-[15px] py-[6px] lg:py-[12px]',
              { 'pointer-events-none opacity-20': zoom === 8 }
            )}
          />
        </div>
      </div>
