...
export default function Credits() {
    ...
    if (isLoaded) {
        return (
            <motion.div style={{ background: `url(${ bgBlack }) center / cover no-repeat`}} className="px-[20px] sm:px-0" exit={{opacity: 0.999, transition: {duration: siteConfig.curtainsTransitionDuration}}} id="credits">
                <div className='container mx-auto h-[calc(100dvh-120px)] sm:h-[calc(100vh-120px)] flex flex-col overflow-hidden relative'>
                    <div className={`credits-animation absolute w-full ${animationEnded ? 'stopped' : ''}`} onAnimationEnd={handleAnimationEnd}>
                ...
                { animationEnded &&
                    <div className='h-[100vh] absolute inset-0 z-[201] flex items-center justify-center bg-black'>
